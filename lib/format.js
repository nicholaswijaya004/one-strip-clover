...
function validateOrder(body, opts = {}) {
  ...
  if (!b.consent) return "CONSENT_REQUIRED";
  if (!Array.isArray(b.photos) || b.photos.length === 0) return "NO_PHOTOS";
  if (b.photos.length > maxPhotos) return "TOO_MANY";
  if (!b.name || !String(b.name).trim()) return "NAME_REQUIRED";
  if (!b.email && !b.wa && !b.contact) return "CONTACT_REQUIRED";
  if (b.email && !EMAIL_RE.test(String(b.email).trim())) return "BAD_EMAIL";
  if (b.wa && !WA_RE.test(String(b.wa).trim())) return "BAD_WA";
  if (!b.address || String(b.address).trim().length < 12) return "ADDRESS_REQUIRED";
  return null;
}
