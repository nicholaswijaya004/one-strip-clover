...
async function sendEmail(o, att, env = process.env) {
  ...
  /* Pengirim SELALU alamatmu sendiri (Gmail menolak "from" yang berbeda dari
     akun yang login). Email pembeli dipakai sebagai replyTo, supaya kamu
     tinggal menekan Reply untuk membalas ke pembeli — bukan ke dirimu sendiri.
     Kalau pembeli hanya mengisi WhatsApp, replyTo dikosongkan. */
  const emailPembeli = String(o.email || "").trim();
  const balasKe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailPembeli) ? emailPembeli : null;
