...
test('render: menolak jumlah foto yang tidak lengkap', async () => {
  await assert.rejects(() => render.renderStrip({ photos: ['a', 'b'] }), /butuh 3 foto|belum terpasang|tidak aktif/);
});
...
test('strip: jumlah foto tidak ditulis ulang di index.html', () => {
  ...
  assert.ok(index.includes('StripRenderer.PHOTO_COUNT'), 'halaman harus mengambil jumlah foto dari modul bersama');
});
...
function mockCtx(kotak) {
  ...
  return {
    ...
    translate(x, y) {
      mul({ a: 1, b: 0, c: 0, d: 1, e: x, f: y });
    },
    rotate(t) {
      mul({ a: Math.cos(t), b: Math.sin(t), c: -Math.sin(t), d: Math.cos(t), e: 0, f: 0 });
    },
    scale(x, y) {
      mul({ a: x, b: 0, c: 0, d: y, e: 0, f: 0 });
    },
    fillRect(x, y, w, h) {
      const p1 = titik(x, y),
        p2 = titik(x + w, y + h);
...
test('strip: rasio foto SAMA di HP & laptop (hasil cetak tidak berbeda)', () => {
  ...
  assert.ok(!index.includes('matchMedia("(max-width:560px)").matches ? 1 : 4/3'), 'rasio tidak boleh bergantung lebar layar');
  assert.ok(index.includes('LAYOUT_BAWAAN.photoWmm / StripRenderer.LAYOUT_BAWAAN.photoHmm'), 'rasio diturunkan dari ukuran kotak foto (5:4), bukan angka tetap');
});
...
test('strip: rasio mengikuti kotak foto template kalau ada', () => {
  ...
  assert.ok(index.includes('terapkanRasioFoto'), 'harus ada penyesuaian rasio dari layout');
  assert.ok(index.includes('L.photoWmm / L.photoHmm'), 'rasio dihitung dari ukuran kotak foto');
});
...
test('renderer: foto kurang tidak membuat seluruh render gagal', () => {
  ...
  const ctx = mockCtx(kotak);
  const img = { width: 1000, height: 1000 };
  assert.doesNotThrow(() => renderer.drawStrip(ctx, [img], { width: 600, aspect: 1, frameId: 'clover' }));
});

test('pengiriman: lembar A4 jadi lampiran PERTAMA (yang dicetak staf)', () => {
  const delivery = fs.readFileSync(path.join(__dirname, '..', 'lib', 'delivery.js'), 'utf8');
...
test('kamera: rasio pratinjau = rasio kotak foto strip (5:4)', () => {
  ...
  assert.ok(index.includes('StripRenderer.LAYOUT_BAWAAN.photoWmm / StripRenderer.LAYOUT_BAWAAN.photoHmm'), 'ASPECT harus diturunkan dari ukuran kotak foto, bukan angka tetap');
});
...
test('halaman: template muncul sebagai pilihan di baris Bingkai', () => {
  ...
  const out = execFileSync('node', [path.join(__dirname, '..', 'scripts', 'simulasi-browser.js')], { encoding: 'utf8', env: { ...process.env, UJI_TEMPLATE: '1' } });
...
test('halaman: simulasi browser ketat lulus semua', () => {
  ...
  const { execFileSync } = require('child_process');
  const out = execFileSync('node', [path.join(__dirname, '..', 'scripts', 'simulasi-browser.js')], { encoding: 'utf8' });
...
test('halaman pembuka: punya tombol masuk ke /booth', () => {
  const depan = fs.readFileSync(path.join(__dirname, '..', 'public', 'index.html'), 'utf8');
